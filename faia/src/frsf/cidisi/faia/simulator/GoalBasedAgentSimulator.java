...
public abstract class GoalBasedAgentSimulator extends Simulator {
    ...
    public void start() {
        ...
        do {
            ...
            System.out.println("\t\t >>>ENVIADO PERCEPCIONES AL AGENTE...<<<");
            perception = this.getPercept();
            agent.see(perception);
            System.out.println("\t\t >>> PERCEPCI”N DEL AGENTE<<<" + perception);

            System.out.println("\t\t >>> ESTADO DEL AGENTE<<< " + agent.getAgentState());
            System.out.println("\t\t >>> ESTADO DEL AMBIENTE<<< " + environment);

            System.out.println("\t\t >>> PENSANDO ACCI”N A REALIZAR...<<<");
            ...
            System.out.println("\t\t >>> ACCI”N A REALIZAR => \t " + action);
            System.out.println();
        ...
        if (this.agentSucceeded(action)) {
            System.out.println("\t\t >>>EL AGENTE HA LLEGADO A LA META<<<");
        } else {
            System.out.println("\t\t >>>LA SIMULACION TERMINO Y EL AGENTE NO LLEG” A LA META<<<");
            System.out.println("\t\t\t =[ =[ ");
        }
